...
class PriorityQueue:
    ...
    def append(self, key):
        ...
        parent = (len(self.queue) -1)/2
        key_index = self.__len__()

        while parent >= 1:
            #compare parent with key, if key is lesser then swap
            if self.queue[parent] > key:
                self.queue[parent], self.queue[key_index] = key, self.queue[parent]
                key_index = parent
                parent = parent/2 
            else:
                break 
    ...
    def min(self):
        ...
        if len(self.queue) == 0 or len(self.queue) == 1:
            return None
        #self._find_min()
    ...
    def pop(self):
        ...
        #self.min_index = None
        self.queue[1], self.queue[self.__len__()] = self.queue[self.__len__()], self.queue[1]
        ...
        k = self.__len__() 
        while (k>=2*parent):  
            if k>=2*parent+1 and self.queue[parent] > self.queue[rightChild] and self.queue[rightChild]< self.queue[leftChild]:
                self.queue[parent], self.queue[rightChild] = self.queue[rightChild], self.queue[parent]
                parent = rightChild
                leftChild, rightChild = 2*parent, 2*parent + 1
            elif self.queue[parent] > self.queue[leftChild]:
                self.queue[parent], self.queue[leftChild] = self.queue[leftChild], self.queue[parent]
                parent = leftChild
                leftChild, rightChild = 2*parent, 2*parent + 1
            else:
                break 
