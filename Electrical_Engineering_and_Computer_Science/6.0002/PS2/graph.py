...
class WeightedEdge(Edge):
    ...
    def __str__(self):
        return '{}->{} ({}, {})'.format(self.get_source().get_name(), self.get_destination().get_name(), self.get_total_distance(), \
                                self.get_outdoor_distance())
...
class Digraph(object):
    ...
    def add_edge(self, edge):
        ...
        if self.has_node(edge.get_source()) and self.has_node(edge.get_destination()):
            ...
            raise ValueError('Nodes are not present. Please verify')
    ...
    def get_Edge(self, start, end): 
        src = self.get_Node_from_name(start)
        dest = self.get_Node_from_name(end)
