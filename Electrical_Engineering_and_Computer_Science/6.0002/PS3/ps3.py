...
class RectangularRoom(object):
    ...
    def clean_tile_at_position(self, pos, capacity):
        ...
        if self.tiles[(math.floor(pos.get_x()), math.floor(pos.get_y()))] < 0:
...
class Robot(object):
    ...
    def __init__(self, room, speed, capacity):
        ...
        self.pos = Position(random.randint(0, room.width-1), random.randint(0, room.height-1))
...
class FurnishedRoom(RectangularRoom):
    ...
    def get_random_position(self):
        ...
        """
        import random
        valid_x = set(range(self.width)) - set([x for x,y in self.furniture_tiles])
        valid_y = set(range(self.height)) - set([y for x,y in self.furniture_tiles])
        return Position(random.choice(valid_x), random.choice(valid_y))
...
class StandardRobot(Robot):
    ...
    def update_position_and_clean(self):
        ...
        """
        new_pos = self.pos.get_new_position(self.get_robot_direction(), self.speed)
...
class FaultyRobot(Robot):
    ...
    def gets_faulty(self):
        ...
        """
        faulty = random.random() < FaultyRobot.p
        return faulty
...
def run_simulation(num_robots, speed, capacity, width, height, dirt_amount, min_coverage, num_trials,
    ...
    for i in range(num_trials): 
        ...
        for each in range(num_robots):  
            robots[each] = robot_type(room, speed, capacity) 
        while True:
            for each in range(num_robots):
                robots[each].update_position_and_clean()
            #print(room.get_num_cleaned_tiles(),room.get_num_tiles()) 
            if float(room.get_num_cleaned_tiles())/room.get_num_tiles() >= min_coverage: 
                end = time.time()
...
#       of a 20x20 room?
# Based on the graph, the time taken by faulty robot is always higher than time taken by STandard robot. It is understandable
# as Faulty robot performs suboptimally
#
# 2) How does the performance of the two robot types compare when two of each
#       robot cleans 80% of rooms with dimensions 
#       10x30, 20x15, 25x12, and 50x6?
#Both the robots show the same trend. i.e as aspect ratio increases timetaken to clean also increases. However the timetaken by faulty
#robot is always higher than time taken by Standard Robot. This is understandable as faulty robot performs sub optimally.
#
